...
"[project]/.next-internal/server/app/api/auth/verify-code/route/actions.js [app-rsc] (server actions loader, ecmascript)": (function(__turbopack_context__) {
...
"[project]/app/api/auth/verify-code/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const { data: codeData, error: codeError } = await supabase.from('email_codes').select('*').eq('email', email).eq('code', code).eq('used', false).gt('expires_at', new Date().toISOString()).single();
        if (codeError) {
        ...
        const { error: updateError } = await supabase.from('email_codes').update({
            used: true
        }).eq('id', codeData.id);
        if (updateError) {
        ...
        const tempPassword = (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$lib$2f$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateRandomString"])(32);
        // Create or update the user with the temporary password
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6f12512c._.js.map